...
$(document).ready(function(){    
	...
	$guardar.click(function(event){
        ...
        $cargar.click(function(event){
            event.preventDefault();            
            titulo=$("select option:selected").text();
            cargar();
        });
...
function ajustarDraggableContenedorRemovido(){
    ...
    function cargar(){
                $.ajax({
                type: "GET",
                url: "../IF_Tomcat/xml/Modulo_Profesor.xml",
                dataType: "xml",           
                success:function(xml){
                    $(xml).find('usuario').each(function(index) {
                        if($(this).find('nombre').text()==nombre_profesor){
                            if($(this).find('titulo').text()==titulo){
                                nodo=index;
                                destruirObjetosDraggables(contador_figura);
                                destruirContenedorFigura(contador_contenedores);
                                destruirFiguraDND(contador_denominador,contador_contenedores);
                                
                                contador_numerador=1;
                                contador_figura=parseInt($(this).find('contador_figura').text());
                                contador_denominador=parseInt($(this).find('contador_denominador').text())+1;
                                contador_contenedores=parseInt($(this).find('contador_contenedores').text())-1;
                                codigo_contenedor_principal=$(this).find('contenedor_principal').text();
                                codigo_contenedor_objetos_draggables=$(this).find('contenedor_principal').text();   
                                controladorDenominadorQuitarBoton();
                                controladorFlechaAgregarContenedor();
                                controladorNumeradorQuitarBoton();
                ...
                success:function(xml){
                    ...
                    $(xml).find("usuario").each(function(){
                        if(nombre_profesor==$(this).find("nombre").text()){                                
                                $("<option value='"+$(this).find('titulo').text()+"'>"+$(this).find('titulo').text()+"</option>").appendTo('select');  
                    }   
                    });
                }
            });
		...
		$.ajax({ //Metodo para enviar datos al servidor                       
                    type: "POST",//Se enviaran los datos usando el metodo post                 
                    url: "../IF_Tomcat/EliminarFracciones",//a donde se enviaran                     
                    data: {nodo_usuario:nodo},
                    success:function(resp){
                        cargarOption();
                        alert("Eliminado");
                    }
        ...
        function nuevoTrabajo(titulo){
                $.ajax({ //Metodo para enviar datos al servidor                       
                    type: "POST",//Se enviaran los datos usando el metodo post                 
                    url: "../IF_Tomcat/NuevaFraccion",//a donde se enviaran                     
                    data: {nombre:nombre_profesor,titulo:titulo},
                    success:function(resp){
                        alert("Creado");
                    }
                });
