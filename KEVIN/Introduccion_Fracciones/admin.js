$(document).ready(function(){
		...
		var $cabecera_usuario=$("#cabecera_usuario").text();
                var $guardar_usuario=$('#btn_add_user');
                var $nombre_usuario;
		...
		$(".actualizar_datos").on( "click", function() {
			...
			$.post('../IF_Tomcat/xml/Usuarios.xml', function(d){ //metodo .post  de ajax en jquery 
    	...
    	$("#validationForm").validate({ //funcion de jQuery para validar formularios, en este caso el de agregar usuarios
			    rules: {
			      nombre: {
			      	...
			      	minlength: 4,
			      	remote:{
			      		url: "../IF_Tomcat/UsuariosExistentesAjax",
      					type: "post",
      					data: {
			          		nombre: function() {
		    ...
		    submitHandler: function(form) {
		      form.submit();
                ...
                function crearUsuario(){
                    $.ajax({                
                        type: "POST",
                        url: "../IF_Tomcat/AgregarProfesor",
                        data: {userName:$nombre_usuario},
